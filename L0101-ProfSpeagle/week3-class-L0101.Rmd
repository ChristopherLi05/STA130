...
author: "Nathalie Moon & Josh Speagle"
date: "2023-01-22"
output:
...
baby_names_m <- baby_names_untidy %>% 
  select(year, male, rank) %>% 
  mutate(sex = "male") %>% 
  rename(name = male)
baby_names_f <- baby_names_untidy %>% 
  select(year, female, rank) %>% 
  mutate(sex = "female") %>% 
  rename(name = female)
...
survey <- gss2010 %>% 
  filter(!is.na(hrsrelax) & !is.na(mntlhlth) & !is.na(hrs1) 
         & !is.na(degree) & !is.na(grass)) %>%
  ...
  mutate(degree = ifelse(degree=="LT HIGH SCHOOL", 
                         yes="LESS THAN HIGH SCHOOL", no=degree))
...
baby_names %>% filter(name == "Josh")
baby_names %>% filter(name %in% c("Joshua", "Josh"))
baby_names %>% filter(name == "Josh" | name == "Joshua")
```

```{r}
# Let's simplify our results table to just see the columns we care about: year
baby_names %>% 
  filter(name == "Josh" | name == "Joshua") %>%
  select(year)
```
...
# (If there are multiple, the first will be chosen.)
baby_names %>%  
  filter(name == "Josh" | name == "Joshua") %>%
  arrange(rank) %>%
  select(year) %>%
...
```{r}
# Can we calculate the proportion of years (in our data) 
# that each name was in the top 20?
baby_names %>%
  group_by(name, sex) %>%
  summarise(n=n()) %>%
  arrange(desc(n)) %>%
...
## baby_names: `filter()`, `select()`, and `arrange()`
```{r}
baby_names <- read_csv("babynames.csv")
...
# Extract only the names most popular each year
...
```{r}
# Calculate summary statistics for the number of hours worked per week
survey %>%
  summarise(
    n = n(),
...
```{r}
# What about calculating the above for each level of education separately?
survey %>%
  group_by(degree) %>%
  summarise(
    n = n(),
...
```{r}
# Let's add a new variable that is
# the ratio of hours spent relaxing and hours spent working
survey <- survey %>%
  mutate(ratio_relax_to_work = hours_relax_per_day * 7 / hours_worked_per_week)

glimpse(survey)

# Let's compute some summaries of it
survey %>%
  group_by(degree) %>%
  summarise(
    n = n(),
...
```{r}
# Now look at standard deviation too and also reorder degree
survey %>% group_by(degree) %>%
...
survey %>% 
  mutate(workload = case_when(hours_worked_per_week < 40 ~ "< 40hrs",
                              hours_worked_per_week > 40 ~ "> 40hrs")) %>%
  group_by(workload) %>%
  summarise(n = n(),
        prop_approve = sum(marijuana == "LEGAL") / n)
